...
So, why is this? Well, a simple answer is provided in [the previous blog post]({% post_url 2025-12-28-why-does-glfwprovider-checkformainthread-exist %}): Most GLFW APIs need to be called on the main thread. And finalizers in C# are not run on the main thread, they are run in their own special [finalizer thread](https://devblogs.microsoft.com/dotnet/finalization-implementation-details/). This means that we cannot call GLFW functions in the finalizer, which means we can't destroy the GLFW window. So, instead we throw an exception to hopefully notify the programmer of their wrongdoing.
